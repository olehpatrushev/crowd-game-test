...
function PIXIText(_text, _style) {
    ...
    for (var i = 0; i < _text.length; i++) {
        if (_text[i] != " " && _text[i] != "	") {
            if (appFonts[_style.fontFamily] && appFonts[_style.fontFamily]["rect_" + _text[i]]) {
            } else {
                isBitmap = false;
            }
        }
    ...
    if (!_style.valign) {
        _style.valign = "center";
    }
    if (!_style.wordWrap) {
        _style.wordWrap = false;
    }
    if (!_style.wordWrapWidth) {
        _style.wordWrapWidth = 720;
    }
    if (!_style.wordWrapHeight) {
        _style.wordWrapHeight = 720;
    }
...
function RandomCash() {
    idRandomCash++;
    if (idRandomCash == aRandomCash.length) {
        idRandomCash = 0;
    }
